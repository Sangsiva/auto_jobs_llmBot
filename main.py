...
from src.linkedin.application_tracker import print_report, update_status
...
def handle_inquiries(selected_actions: List[str], parameters: dict, llm_api_key: str):
    ...
    try:
        if selected_actions:
            ...
            if "Generate Tailored Resume from JD (Smart Match)" == selected_actions:
            ...
            if "Auto Apply to LinkedIn Jobs" == selected_actions:
                ...
                if not linkedin_email or "TODO" in linkedin_email:
                    print("ERROR: Add your linkedin_email and linkedin_password to data_folder/secrets.yaml first.")
                else:
                    # Load plain text resume for profile info
                    import yaml
                    ...
                    if resume_pdf:
                        try:
                            ...
                            driver = init_browser()
                            print("Browser ready. Starting bot...")
                            bot = LinkedInBot(
                                driver=driver,
                                api_key=llm_api_key,
...
ACTION_MAP = {
    ...
    "resume-jd":    "Generate Resume Tailored for Job Description",
    ...
    try:
        questions = [
            inquirer.List(
                ...
                choices=[v for v in ACTION_MAP.values()],
            ),
        ]
...
def main():
    ...
    import argparse
    parser = argparse.ArgumentParser(description="AIHawk Job Application Bot")
    parser.add_argument(
        "--action", "-a",
        choices=list(ACTION_MAP.keys()),
        help=(
            "Action to run directly without interactive menu. "
            "Choices: " + ", ".join(f"{k} ({v})" for k, v in ACTION_MAP.items())
        ),
    )
    ...
    try:
        ...
        # Use CLI argument if provided, otherwise show interactive menu
        if args.action:
            selected_actions = ACTION_MAP[args.action]
            print(f"Running: {selected_actions}")
        else:
